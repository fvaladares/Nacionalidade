 ...
 * Last modified 09/08/2022 07:51
 *
...
import br.pucminas.meunome.databinding.FragmentResultadoBinding
...
class ResultadoFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        _binding = FragmentResultadoBinding.inflate(
            inflater,
            container,
            false
        )
    ...
    private fun exibirRecyclerView(country: List<Pais>) {
        binding.rvNomes.apply {
            layoutManager = LinearLayoutManager(requireContext())
            adapter = OrigemNomeAdapter(country)
        }
    }

    private fun prepararObservers() {
        sharedViewModel.origemNome.observe(viewLifecycleOwner) {
            it?.country?.let { it1 -> exibirRecyclerView(it1) }
        }
    }
}